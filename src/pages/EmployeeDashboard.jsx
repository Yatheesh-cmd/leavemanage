...
function EmployeeDashboard() {
  ...
  useEffect(() => {
    setLocalLeaves(leaves) // Sync localLeaves with Redux store leaves
  }, [leaves])
  ...
  const onSubmit = async (data) => {
    ...
    // Check if leave duration exceeds 20 days
    if (leaveDays > 20) {
      toast.error('Leave duration cannot exceed 20 days')
      return
    }

    // Check yearly leave limit (20 days)
    ...
    if (yearlyLeaveDays >= 20) {
      toast.error('Yearly leave quota exceeded (20 days/year)')
      return
    }

    if (yearlyLeaveDays + leaveDays > 20) {
      toast.error('Requested leave exceeds yearly quota of 20 days')
      return
    }
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
          ...
          <div className="p-6">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full bg-teal-600 hover:bg-teal-700 text-white py-2 px-4 rounded-lg transition flex items-center justify-center gap-2"
